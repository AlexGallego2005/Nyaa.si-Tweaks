    ...
    <table class="table table-bordered table-hover table-striped torrent-list">
        <thead>
            <tr>
                ...
                <th class="hdr-date text-center" title="In local time" style="width:140px;">Download Date</th>
...
async function loadDownloads()
{
    ...
    const downloads = await load('downloads');
...
<td class="text-center" data-timestamp="${ download.start }">${ download?.start?.split('T')?.[0] } ${ download?.start?.split('T')?.[1]?.split('.')?.[0] }</td>
